...
describe('CircularReferenceResolver', () => {
  ...
  describe('getInitialData', () => {
    it('should return data without circular reference fields', () => {
      const entity = {
        name: 'user1',
        fixture: {
          data: {
      ...
      const result = resolver.getInitialData(entity, {});
      
      expect(result.name).toBe('John');
      expect(result.email).toBe('john@example.com');
      expect(result.managerId).toBe('@user3');
      expect(result).not.toHaveProperty('friendId');
    });

    it('should return empty object when fixture has no data', () => {
      const entity = {
        name: 'user1',
    ...
    it('should handle nested field removal', () => {
      ...
      expect(result.name).toBe('John');
      expect(result.profile.bio).toBe('User bio');
      expect(result.profile).not.toHaveProperty('friendId');
    });
  });
